...
describe('Royalty', function () {
  ...
  it('Should be able to add address of payee', async () => {
    //GIVEN
    const sharePayee4 = 10
    const payeeToAdd = this.daemon.address

    const payeeToChangeShare = this.payeeG1[0]
    await this.royalty.removePayee(0, payeeToChangeShare)
    await this.royalty.addPayee(payeeToChangeShare, this.shareG1[0] - sharePayee4)

    const oldTotalShare = await this.royalty.totalShares()
    //WHEN
    await this.royalty.addPayee(payeeToAdd, sharePayee4)
    const actualPayee4 = await this.royalty.payee(3)
    const sharesPayee4 = await this.royalty.shares(payeeToAdd)
    const actualTotalShare = await this.royalty.totalShares()
    //THEN
    expect(actualPayee4).to.be.equal(payeeToAdd)
    expect(sharesPayee4.toString()).to.be.equal(sharePayee4.toString())
    expect(actualTotalShare).to.be.equal(BigNumber.from(oldTotalShare).add(sharePayee4))
  })
  ...
  it('Should be able to distributes funds to payees', async () => {
    ...
    await this.WETH.transfer(this.royalty.address, amountToSend)
    ...
    const expectAmountPayee1 = (this.shareG1[0] * amountToSend) / 100
    const expectAmountPayee2 = (this.shareG1[1] * amountToSend) / 100
    const expectAmountPayee3 = (this.shareG1[2] * amountToSend) / 100
